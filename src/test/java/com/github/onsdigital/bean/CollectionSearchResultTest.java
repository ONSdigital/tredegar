...
public class CollectionSearchResultTest {
	...
	public void testGetResults() {
		...
		assertEquals("title should be available from map", "Producer Price Inflation", bulletin.get("title"));
		assertEquals("release date should be available from map", "19 February 2014", bulletin.get("releaseDate"));
		assertEquals("url should be available from map", "/taxonomy/economy/inflationandpriceindices/bulletins/producerpriceinflation/2014-09-21/", bulletin.get("url"));
	...
	private List<File> setUpFiles(String fileName) {
		...
		if (StringUtils.isEmpty(fileName)) {
			file = new File("target/taxonomy/economy/inflationandpriceindices/bulletins/producerpriceinflation/2014-09-21/data.json");
		} else {
