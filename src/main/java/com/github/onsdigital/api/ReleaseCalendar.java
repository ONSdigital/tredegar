...
public class ReleaseCalendar {
	...
	public void get(@Context HttpServletRequest httpServletRequest, @Context HttpServletResponse httpServletResponse) throws IOException {
		...
		httpServletResponse.setHeader("Content-Disposition", "attachment; filename=releases.ics");
		IOUtils.copy(new StringReader(CALENDAR_EVENT), httpServletResponse.getOutputStream());
