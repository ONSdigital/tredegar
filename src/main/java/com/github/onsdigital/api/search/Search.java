...
public class Search {
	...
	private SearchResult searchSuggestions(String query, int page, String[] types, SearchResult searchResult) throws IOException, Exception {
		...
		if (StringUtils.isEmpty(suggestionsBufferAsString)) {
			...
			ONSQueryBuilder suggestionsQueryBuilder = new ONSQueryBuilder("ons").setTypes(types).setPage(page).setSearchTerm(suggestionsBufferAsString).setFields(getTitle(), "url");
	...
	private SearchResult searchTimeseries(String query, int page) throws Exception, IOException {
		...
		ONSQueryBuilder timeSeriesQueryBuilder = new ONSQueryBuilder("ons").setType(ContentType.timeseries.name()).setPage(page).setSearchTerm(query).setFields(getTitle(), "url");
