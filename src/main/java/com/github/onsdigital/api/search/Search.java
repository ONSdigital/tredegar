...
public class Search {
	...
	private Object search(String query, int page, String type) throws Exception {
		...
		if (searchResult.getNumberOfResults() == 0 && type == null) {//If type is set don't search for timeseries
			...
			if (searchResult.getNumberOfResults() == 0) {
				searchResult = searchSuggestions(query, page, type, searchResult);
			}
		}
		return searchResult;
	}
	...
	private SearchResult searchSuggestions(String query, int page, String type, SearchResult searchResult) throws IOException, Exception {
		...
		if (StringUtils.isEmpty(suggestionsBufferAsString)) {
			...
			ONSQueryBuilder suggestionsQueryBuilder = new ONSQueryBuilder("ons").setType(type).setPage(page).setSearchTerm(suggestionsBufferAsString).setFields(getTitle(), "path");
