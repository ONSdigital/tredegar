...
public class XLSXGenerator {
	...
	public XLSXGenerator(List<Timeseries> timeseries,
			Map<String, TimeseriesValue[]> data) {
		...
		for (Entry<String, TimeseriesValue[]> rowValues : data.entrySet()) {
			...
			for (TimeseriesValue timeseriesValue : rowValues.getValue()) {
				row.createCell(i++).setCellValue(
						timeseriesValue == null ? null : timeseriesValue.value);
			}
		}

	}

	private void generateHeaders(Workbook wb, Sheet sheet,
		...
		for (Timeseries timeseries : this.timeseries) {
			...
			System.out.println("Geneararing XLSX for: " + timeseries.name
					+ " at: " + timeseries.uri);
		}
	}
}
